...
+(function($) {
    ...
    var Tab = function(element, options) {
        ...
        this.tabId =
            "tabs" + $el.parents().length + Math.round(Math.random() * 1000);

        if (
            this.options.closable !== undefined &&
    ...
    Tab.prototype.init = function() {
        ...
        this.options.slidable =
            this.options.slidable !== undefined &&
            this.options.slidable !== false;
        ...
        this.$el.on("close.oc.tab", function(ev, data) {
            ev.preventDefault();
            var force =
                data !== undefined && data.force !== undefined
                    ? data.force
                    : false;
            self.closeTab(
                $(ev.target).closest("ul.nav-tabs > li, div.tab-content > div"),
                force
            );
        });

        this.$el.on("mousedown", "li[data-tab-id]", function(ev, data) {
            if (2 == ev.which) {
                ev.preventDefault();
                var force =
                    void 0 !== data && void 0 !== data.force && data.force;
                self.closeTab(
                    $(ev.target).closest(
                        "ul.nav-tabs > li, div.tab-content > div"
                    ),
                    force
                );
            }
    ...
    Tab.prototype.initTab = function(li) {
        var $tabs = $(">li", this.$tabsContainer),
        ...
        if (!$anchor.attr("title")) $anchor.attr("title", $anchor.text());

        var html = $anchor.html();

        $anchor.html("");
        $anchor.append(
            $('<span class="title"></span>').append(
                $("<span></span>").html(html)
            )
        );

        var pane = $("> .tab-pane", this.$pagesContainer)
            .eq(tabIndex)
            .attr("id", targetId);

        if (!$("span.tab-close", li).length) {
            $(li).append(
                $('<span class="tab-close"><i>&times;</i></span>').click(
                    function() {
    ...
    Tab.prototype.addTab = function(title, content, identifier, tabClass) {
        var processedTitle = this.generateTitleText(title, -1),
            $link = $("<a/>")
    ...
    Tab.prototype.generateTitleText = function(title, tabIndex) {
        ...
        if (
            this.options.maxTitleSymbols &&
            newTitle.length > this.options.maxTitleSymbols
        )
            newTitle =
                "..." +
                newTitle.substring(
    ...
    Tab.prototype.closeTab = function(tab, force) {
        ...
        var $tab = $("> li", this.$tabsContainer).eq(tabIndex),
            $pane = $("> div", this.$pagesContainer).eq(tabIndex),
        ...
        if (
            isModified &&
            this.options.closeConfirmation !== undefined &&
            force !== true
        ) {
            if (!confirm(this.options.closeConfirmation)) return;
        }
