 ...
 * - Adds keyboard navigation controls to checkbox and checkbox list
 */
$(function() {
    "use strict";
    ...
    */
    ...
    $('body').on('keydown', '.checkbox.custom-checkbox', function(event) {
        // Run whitelist checker
        whiteList(event.key);
        // Get current checkbox dom location
        var $target = $(event.currentTarget).find('input[type="checkbox"]');

        if (event.key === 'Enter' || event.key === '(Space character)' || event.key === 'Spacebar' || event.key === ' ') {
            event.preventDefault();
    ...
    /*
     * Checkbox and checkbox list - mouse control
     */
    $('body').on('click', '.checkbox.custom-checkbox input[type="checkbox"]', function(event) {
        // Get current checkbox dom location
        var $target = $(event.currentTarget);
        // Turn off focus ring when a user clicks a checkbox with the mouse
        $target.closest('.checkbox.custom-checkbox').removeClass('focus-visible');
