...
for(var i=0,len=headerCells.length;i<len;i++){if(dataCells[i]&&headerCells[i].getAttribute('style')){dataCells[i].setAttribute('style',headerCells[i].getAttribute('style'))}}}
...
ComputedProcessor.prototype.dispose=function(){BaseProto.dispose.call(this)
this.focusTimeoutHandler=null}
ComputedProcessor.prototype.renderCell=function(value,cellContentContainer){var cellElement=cellContentContainer.parentNode
cellElement.classList.add('muted')
this.createViewContainer(cellContentContainer,value)
if(!value){value=this.getCachedValue(cellElement)
if(value){this.setViewContainerValue(cellElement,value)}else{this.getComputedValue(cellElement)}}}
ComputedProcessor.prototype.isCellFocusable=function(){return false}
ComputedProcessor.prototype.keyNavigationAllowed=function(ev,direction){return true}
ComputedProcessor.prototype.onRowValueChanged=function(columnName,cellElement){var dependsOn=this.columnConfiguration.dependsOn
if(!dependsOn){return}
if(typeof dependsOn==='object'){for(var i in dependsOn){if(columnName===dependsOn[i]){this.getComputedValue(cellElement)
break}}}else if(columnName===dependsOn){this.getComputedValue(cellElement)}}
ComputedProcessor.prototype.getComputedValue=function(cellElement){var row=cellElement.parentNode,that=this,viewContainer=this.getViewContainer(cellElement)
viewContainer.setAttribute('class','loading')
var requestData={rowData:this.tableObj.getRowData(row)}
this.tableObj.$el.request(this.getHandlerName(),{data:requestData}).done(function(data){if(data.result){that.setViewContainerValue(cellElement,data.result)
that.setCachedValue(cellElement,data.result)}})}
ComputedProcessor.prototype.getCacheKey=function(cellElement){return'value-'
+this.tableObj.getAlias()+'-'
+this.columnName+'-'
+this.tableObj.getCellRowKey(cellElement)}
ComputedProcessor.prototype.getCachedValue=function(cellElement){var cacheKey=this.getCacheKey(cellElement);if(this.cachedValues.hasOwnProperty(cacheKey)){return this.cachedValues[cacheKey]}
return null}
ComputedProcessor.prototype.setCachedValue=function(cellElement,value){this.cachedValues[this.getCacheKey(cellElement)]=value}
