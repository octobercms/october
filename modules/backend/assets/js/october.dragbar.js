...
/*
 * October dragbar code - needs to be placed at the bottom of the webpage
 */
var dragbarOne = document.getElementsByClassName('split-a1'),
    dragbarTwo = document.getElementsByClassName('split-a2'),
    sizesOne = localStorage.getItem('split-sizes-one'),
    sizesTwo = localStorage.getItem('split-sizes-two');

// Setting page sidebar - element with class "split-a1" exist
if (dragbarOne.length > 0) {
    if (sizesOne) {
        sizesOne = JSON.parse(sizesOne)
    } else {
        sizesOne = [1, 99] // default sizes
    }
    
    Split(['.split-a1', '.split-b'], {
        sizes: sizesOne,
        onDragEnd: function(sizes) {
            localStorage.setItem('split-sizes-one', JSON.stringify(sizes))
        },
        gutterSize: 15,
        minSize: [300, 85] /* 300 sidbar + 15 gutter + 85 will make 400px to match the media queries */
    });
}

// CMS page and builder plugin sidebar - element with class "split-a2" exist
if (dragbarTwo.length > 0) {
    if (sizesTwo) {
        sizesTwo = JSON.parse(sizesTwo)
    } else {
        sizesTwo = [1, 99] // default sizes
    }    

    Split(['.split-a2', '.split-b'], {
        sizes: sizesTwo,
        onDragEnd: function(sizes) {
            localStorage.setItem('split-sizes-two', JSON.stringify(sizes))
        },
        gutterSize: 15,
        minSize: [350, 100] /* 350px min-width for CMS page sidebar */
    });
}

/*
 * Pinbar - show and hide the cms page sidebar panel
 */
$(function() {
    $("#sidebar-thumb .fix-button").on("click", function(){
        $(".split-b").toggleClass("split-full");
        $(".split-a2").toggle();
    });
});
