    ...
    public function onRelationManageCreate()
    {
        ...
        if ($this->viewMode == 'multi') {
            ...
            foreach ($modelsToSave as $modelToSave) {
                if ($this->controller->methodExists('relationBeforeSave')) {
                    $this->controller->relationBeforeSave($modelToSave, $this->relationName, $this->relationObject);
                }
                if ($this->controller->methodExists('relationBeforeCreate')) {
                    $this->controller->relationBeforeCreate($modelToSave, $this->relationName, $this->relationObject);
                }
                $modelToSave->save(null, $this->manageWidget->getSessionKey());
                if ($this->controller->methodExists('relationAfterSave')) {
                    $this->controller->relationAfterSave($modelToSave, $this->relationName, $this->relationObject);
                }
                if ($this->controller->methodExists('relationAfterCreate')) {
                    $this->controller->relationAfterCreate($modelToSave, $this->relationName, $this->relationObject);
                }
            }
        ...
        elseif ($this->viewMode == 'single') {
            ...
            if ($this->controller->methodExists('relationBeforeSave')) {
                $this->controller->relationBeforeSave($newModel, $this->relationName, $this->relationObject);
            }
            if ($this->controller->methodExists('relationBeforeCreate')) {
                $this->controller->relationBeforeCreate($newModel, $this->relationName, $this->relationObject);
            }
            ...
            if ($this->controller->methodExists('relationAfterSave')) {
                $this->controller->relationAfterSave($newModel, $this->relationName, $this->relationObject);
            }
            if ($this->controller->methodExists('relationAfterCreate')) {
                $this->controller->relationAfterCreate($newModel, $this->relationName, $this->relationObject);
            }
        }
    ...
    public function onRelationManageUpdate()
    {
        ...
        if ($this->viewMode == 'multi') {
            ...
            foreach ($modelsToSave as $modelToSave) {
                ...
                if ($this->controller->methodExists('relationBeforeUpdate')) {
                    $this->controller->relationBeforeUpdate($modelToSave, $this->relationName, $this->relationObject);
                }
                $modelToSave->save(null, $this->manageWidget->getSessionKey());
                if ($this->controller->methodExists('relationAfterSave')) {
                    $this->controller->relationAfterSave($modelToSave, $this->relationName, $this->relationObject);
                }
                if ($this->controller->methodExists('relationAfterUpdate')) {
                    $this->controller->relationAfterUpdate($modelToSave, $this->relationName, $this->relationObject);
                }
            }
        }
        elseif ($this->viewMode == 'single') {
            $this->viewWidget->setFormValues($saveData);
            if ($this->controller->methodExists('relationBeforeSave')) {
                $this->controller->relationBeforeSave($this->viewModel, $this->relationName, $this->relationObject);
            }
            if ($this->controller->methodExists('relationBeforeUpdate')) {
                $this->controller->relationBeforeUpdate($this->viewModel, $this->relationName, $this->relationObject);
            }
            $this->viewModel->save(null, $this->manageWidget->getSessionKey());
            if ($this->controller->methodExists('relationAfterSave')) {
                $this->controller->relationAfterSave($this->viewModel, $this->relationName, $this->relationObject);
            }
            if ($this->controller->methodExists('relationAfterUpdate')) {
                $this->controller->relationAfterUpdate($this->viewModel, $this->relationName, $this->relationObject);
            }
        }
    ...
    public function onRelationManageDelete()
    {
        ...
        if ($this->viewMode == 'multi') {
            if (($checkedIds = post('checked')) && is_array($checkedIds)) {
                foreach ($checkedIds as $relationId) {
                    ...
                    if ($this->controller->methodExists('relationBeforeDelete')) {
                        $this->controller->relationBeforeDelete($obj, $this->relationName, $this->relationObject);
                    }
                    $obj->delete();
                    if ($this->controller->methodExists('relationAfterDelete')) {
                        $this->controller->relationAfterDelete($obj, $this->relationName, $this->relationObject);
                    }
                }
            }
        }
        ...
        elseif ($this->viewMode == 'single') {
            ...
            if ($relatedModel->exists) {
                if ($this->controller->methodExists('relationBeforeDelete')) {
                    $this->controller->relationBeforeDelete($relatedModel, $this->relationName, $this->relationObject);
                }
                $relatedModel->delete();
                if ($this->controller->methodExists('relationAfterDelete')) {
                    $this->controller->relationAfterDelete($relatedModel, $this->relationName, $this->relationObject);
                }
            }
        ...
        if ($this->viewMode == 'multi') {
            ...
            if (is_array($checkedIds)) {
                ...
                foreach ($models as $model) {
                    if ($this->controller->methodExists('relationBeforeAdd')) {
                        $this->controller->relationBeforeAdd($model, $this->relationName, $this->relationObject);
                    }
                    $this->relationObject->add($model, $sessionKey);
                    if ($this->controller->methodExists('relationAfterAdd')) {
                        $this->controller->relationAfterAdd($model, $this->relationName, $this->relationObject);
                    }
                }
            }
        }
    ...
    public function onRelationManageRemove()
    {
        ...
        if ($this->viewMode == 'multi') {
            ...
            if (is_array($checkedIds)) {
                ...
                foreach ($models as $model) {
                    if ($this->controller->methodExists('relationBeforeRemove')) {
                        $this->controller->relationBeforeRemove($model, $this->relationName, $this->relationObject);
                    }
                    $this->relationObject->remove($model, $sessionKey);
                    if ($this->controller->methodExists('relationAfterRemove')) {
                        $this->controller->relationAfterRemove($model, $this->relationName, $this->relationObject);
                    }
                }
            }
        }
        ...
        elseif ($this->viewMode == 'single') {
            ...
            elseif ($this->relationType == 'hasOne' || $this->relationType == 'morphOne') {
                if ($obj = $relatedModel->find($recordId)) {
                    if ($this->controller->methodExists('relationBeforeRemove')) {
                        $this->controller->relationBeforeRemove($obj, $this->relationName, $this->relationObject);
                    }
                    $this->relationObject->remove($obj, $sessionKey);
                    if ($this->controller->methodExists('relationAfterRemove')) {
                        $this->controller->relationAfterRemove($obj, $this->relationName, $this->relationObject);
                    }
                }
                elseif ($this->viewModel->exists) {
                    if ($this->controller->methodExists('relationBeforeRemove')) {
                        $this->controller->relationBeforeRemove($this->viewModel, $this->relationName, $this->relationObject);
                    }
                    $this->relationObject->remove($this->viewModel, $sessionKey);
                    if ($this->controller->methodExists('relationAfterRemove')) {
                        $this->controller->relationAfterRemove($this->viewModel, $this->relationName, $this->relationObject);
                    }
                }
            }
